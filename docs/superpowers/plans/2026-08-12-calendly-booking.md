...
- **`data-url` on the container is mandatory — do not remove it.** Calendly's `widget.js` auto-scans for `.calendly-inline-widget` elements as soon as it loads and throws on any it finds without a `data-url`. That exception aborts the script *before* it assigns `window.Calendly`, so the manual `initInlineWidget` API is permanently unavailable too and nothing downstream can recover. Dropping the attribute in favour of calling `initInlineWidget` therefore breaks both paths at once, and the failure is near-invisible: the browser reports only a cross-origin `"Script error."` with no file or line. The container now carries `data-url` (auto-scan handles first load) *and* is initialized manually on remount, keyed off the `data-processed` flag Calendly stamps on anything its scan touched so the two paths never double-initialize.
